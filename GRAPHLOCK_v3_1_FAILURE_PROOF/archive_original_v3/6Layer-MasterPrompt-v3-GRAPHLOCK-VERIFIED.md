...
- zero deadlock (every retry loop is bounded; every campaign is durably resumable; every run terminates in ALL_DONE, a truthful release verdict, or a terminal evidence-backed BLOCKED report -- never a silent spin, never a fake DONE),
...
Frontend: [FRONTEND_STACK_OR_UNKNOWN]
Backend: [BACKEND_STACK_OR_UNKNOWN]
Database: [DATABASE_OR_UNKNOWN]
Authentication: [AUTH_OR_UNKNOWN]
Hosting / Deployment: [DEPLOYMENT_OR_UNKNOWN]
Testing: [TESTING_TOOLS_OR_UNKNOWN]
Package Manager: [PACKAGE_MANAGER_OR_UNKNOWN]
CI/CD: [CICD_OR_UNKNOWN]
Observability: [OBSERVABILITY_OR_UNKNOWN]
...
[AGENT_PLATFORMS] (e.g., claude-code, codex, hermes, openclaw, other; "any" is valid)
## Auto-Deploy Authorization
[AUTO_DEPLOY_AUTHORIZED] (yes = the run may deploy to production hands-off once all gates pass; no = the run ends at a proven, tagged, ship-ready artifact and a MANUAL deploy step)
...
## Runtime Budgets
[RUNTIME_BUDGET_NOTES] (optional: wall-clock or step budgets per node; if absent, the FORGE sets sane per-milestone budgets)
...
Auto-remediate verified candidate defects: [AUTO_REMEDIATE_VERIFICATION_FAILURES] (yes/no; default yes for build projects, no for audit-only projects)
Maximum release-candidate epochs: [MAX_RELEASE_CANDIDATE_EPOCHS_OR_DEFAULT_5] (bounds remediation/code-change cycles only; it never shortens required testing inside an epoch. Exhaustion produces an evidence-backed NO_GO with remaining defects.)
Persistent runner available: [PERSISTENT_RUNNER_AVAILABLE]
Maximum safe parallel read-only test jobs: [MAX_PARALLEL_SAFE_TEST_JOBS_OR_AUTO]
Human UAT validators available: [HUMAN_UAT_AVAILABLE]
Manual assistive-technology validators available: [MANUAL_ACCESSIBILITY_VALIDATORS_AVAILABLE]
Accredited external assessors required/available: [EXTERNAL_ASSESSOR_STATUS]
Physical hardware/HSM/device lab available: [HARDWARE_LAB_STATUS]
Production-like environment available: [PRODUCTION_LIKE_ENVIRONMENT_STATUS]
Active production testing authorized: [ALLOW_PRODUCTION_ACTIVE_TESTING] (default no)
Destructive sandbox testing authorized: [ALLOW_DESTRUCTIVE_SANDBOX_TESTING] (default no until containment proof passes)
External network targets authorized: [ALLOW_EXTERNAL_NETWORK_TARGETS] (default no)
Required soak duration: [SOAK_DURATION_OR_REQUIREMENT_DERIVED]
Required fuzzing duration/corpus targets: [FUZZING_BUDGET_OR_RISK_DERIVED]
Required performance/SLO thresholds: [PERFORMANCE_SLO_THRESHOLDS_OR_DISCOVER]
...
EP-010 may execute up to [MAX_RELEASE_CANDIDATE_EPOCHS_OR_DEFAULT_5] candidate epochs. Within an epoch, candidate code and artifact are immutable. After a complete NO_GO accounting, and only when [AUTO_REMEDIATE_VERIFICATION_FAILURES]=yes, GraphLock may open a remediation interval, fix validated candidate defects using the normal milestone laws, create a new commit/artifact digest, increment the epoch, invalidate affected evidence, and rerun the required verification descendants. Harness/environment failures are fixed in the overlay and never used as permission to change production code. Exhausting the epoch cap produces NODE_BLOCKED with all evidence; it never causes gate weakening.
