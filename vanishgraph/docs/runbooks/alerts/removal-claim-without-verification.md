# Runbook - Removal claim without verification; VERIFIED_REMOVED record lacking an observation reference

Alert ids: A-01, A-01b. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-01 - Removal claim without verification** (severity critical, routing page, for 0m)
  * expression: `sum by (source_class) (increase(vanishgraph_truth_state_transitions_total{truth_state="VERIFIED_REMOVED"}[10m])) > sum by (source_class) (increase(vanishgraph_verification_observations_recorded_total[10m]))`
  * threshold: any positive delta
  * WHAT IT MEANS: an unverified removal claim is in the data: more exposures reached VERIFIED_REMOVED in 10 minutes than verification observations were recorded.
  * WHAT IT DOES NOT MEAN: removal is happening. It means the opposite: a claim exists without the observation that would justify it.
* **A-01b - VERIFIED_REMOVED record lacking an observation reference** (severity critical, routing page, for 0m)
  * expression: `sum(increase(vanishgraph_truth_state_transitions_total{truth_state="VERIFIED_REMOVED",has_verification_reference="false"}[5m]))`
  * threshold: > 0
  * WHAT IT MEANS: a VERIFIED_REMOVED transition was recorded with has_verification_reference=false, which SPEC-001 §4.1 T14 forbids.
  * WHAT IT DOES NOT MEAN: the record is merely missing a field. The transition is unlawful without its observation reference.
