...
Alert ids: A-05. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-05 - Cross-tenant access attempt** (severity critical, routing page-security, for 0m)
  * expression: `sum by (layer) (increase(vanishgraph_tenant_scope_refusals_total[5m]))`
  * threshold: > 0
  * WHAT IT MEANS: a cross-tenant access attempt was refused by one of the three layers.
  * WHAT IT DOES NOT MEAN: data was disclosed. Every refused attempt is a denial, and the layer that refused it is the label to read.
