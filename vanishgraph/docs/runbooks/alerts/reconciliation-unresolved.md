# Runbook - Unresolved reconciliation

Alert ids: A-12. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-12 - Unresolved reconciliation** (severity warning, routing ticket, for 15m)
  * expression: `sum by (channel) (increase(vanishgraph_reconciliation_events_total{outcome="UNRESOLVED"}[1h]))`
  * threshold: > 0
  * WHAT IT MEANS: reconciliation events ended UNRESOLVED for one channel in the last hour.
  * WHAT IT DOES NOT MEAN: an effect is missing. Unresolved means the outcome is unknown, and unknown is not success.
