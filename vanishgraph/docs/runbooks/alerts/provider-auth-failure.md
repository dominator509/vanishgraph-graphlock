# Runbook - Provider auth failure; Provider auth failure sustained

Alert ids: A-09, A-09b. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-09 - Provider auth failure** (severity warning, routing ticket, for 10m)
  * expression: `sum by (provider) (increase(vanishgraph_provider_auth_failures_total[10m]))`
  * threshold: > 2
  * WHAT IT MEANS: more than two provider authentication failures in 10 minutes for one provider.
  * WHAT IT DOES NOT MEAN: credentials are wrong. Expired tokens and permission changes look identical here, which is why the reason code is a label.
* **A-09b - Provider auth failure sustained** (severity critical, routing page, for 0m)
  * expression: `sum by (provider) (increase(vanishgraph_provider_auth_failures_total[30m]))`
  * threshold: > 5
  * WHAT IT MEANS: provider authentication failures sustained above five in 30 minutes.
  * WHAT IT DOES NOT MEAN: the provider is down. It means our credentials or entitlements are the likely cause.
