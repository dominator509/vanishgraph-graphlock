# Runbook - DLP egress denial; DLP denial of a prohibited class

Alert ids: A-06, A-06b. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-06 - DLP egress denial** (severity warning, routing ticket, for 10m)
  * expression: `sum by (sink) (increase(vanishgraph_dlp_egress_denied_total[10m]))`
  * threshold: > 5
  * WHAT IT MEANS: the DLP scrub stage denied more than five egress attempts in 10 minutes.
  * WHAT IT DOES NOT MEAN: the product is leaking. Denials are the stage refusing to leak.
* **A-06b - DLP denial of a prohibited class** (severity critical, routing page-security, for 0m)
  * expression: `sum by (egress_class) (increase(vanishgraph_dlp_egress_denied_total{egress_class=~"IDENTITY_DOCUMENT|AUTH_SECRET"}[5m]))`
  * threshold: > 0
  * WHAT IT MEANS: a field classified IDENTITY_DOCUMENT or AUTH_SECRET reached the scrub stage.
  * WHAT IT DOES NOT MEAN: a secret was sent. It was denied at the boundary, and the producer that built it is the defect to find.
