# Runbook - Telemetry identity missing

Alert ids: A-11. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-11 - Telemetry identity missing** (severity warning, routing ticket, for 5m)
  * expression: `sum by (sink, reason_code) (increase(vanishgraph_telemetry_egress_failures_total{reason_code="RESOURCE_ATTR_MISSING"}[10m]))`
  * threshold: > 0
  * WHAT IT MEANS: a process could not resolve its telemetry identity and emitted NO telemetry, with reason_code RESOURCE_ATTR_MISSING.
  * WHAT IT DOES NOT MEAN: telemetry is being lost by the transport. The record was never produced, so fix the missing attribute rather than the counter.
