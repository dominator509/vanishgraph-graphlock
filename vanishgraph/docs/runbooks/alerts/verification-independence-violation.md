# Runbook - Independence violation observed

Alert ids: A-13. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-13 - Independence violation observed** (severity critical, routing page, for 0m)
  * expression: `sum by (source_class) (increase(vanishgraph_verification_observations_recorded_total{path_distinct="false"}[15m]))`
  * threshold: > 0
  * WHAT IT MEANS: a VerificationObservation was recorded with path_distinct=false: the acting path observed itself.
  * WHAT IT DOES NOT MEAN: the observation is weak. VG-VERIFY-001 makes it a violation rather than an observation.
