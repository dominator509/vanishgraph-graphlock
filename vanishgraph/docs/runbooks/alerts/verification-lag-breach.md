# Runbook - Verification-lag breach

Alert ids: A-02. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-02 - Verification-lag breach** (severity warning, routing ticket, for 1h)
  * expression: `histogram_quantile(0.95, sum by (le) (rate(vanishgraph_verification_lag_seconds_bucket[6h]))) > 1209600`
  * threshold: 14 d p95
  * WHAT IT MEANS: the p95 lag from ACKNOWLEDGED to a recorded VerificationObservation exceeded 14 days over 6 hours.
  * WHAT IT DOES NOT MEAN: removals are failing. It means verification is slow, which is a different and slower-burning problem.
