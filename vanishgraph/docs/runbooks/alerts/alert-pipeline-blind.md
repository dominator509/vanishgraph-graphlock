# Runbook - Alert pipeline cannot evaluate

Alert ids: A-15. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-15 - Alert pipeline cannot evaluate** (severity critical, routing page, for 10m)
  * expression: `absent(vanishgraph_truth_state_transitions_total{truth_state="VERIFIED_REMOVED"}) or absent(vanishgraph_readiness_status)`
  * threshold: absent series
  * WHAT IT MEANS: the alerting pipeline cannot evaluate: a required series is absent.
  * WHAT IT DOES NOT MEAN: nothing is wrong. Blindness is the incident, and it is critical for exactly that reason.
