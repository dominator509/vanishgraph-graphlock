# Runbook - Duplicate external effect detected

Alert ids: A-08. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-08 - Duplicate external effect detected** (severity critical, routing page, for 0m)
  * expression: `sum by (channel, detection_path) (increase(vanishgraph_duplicate_external_effect_events_total[30m]))`
  * threshold: > 0
  * WHAT IT MEANS: a second external effect occurred for one IdempotencyKey, detected by one of the three paths.
  * WHAT IT DOES NOT MEAN: the duplicate is harmless. VG-ACTION-001 exists because it is not.
