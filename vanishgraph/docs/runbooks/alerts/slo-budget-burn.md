...
Alert ids: A-14. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-14 - SLO error-budget burn** (severity warning, routing ticket, for 30m)
  * expression: `vanishgraph_slo_error_budget_burn_ratio{objective="availability"} > 2`
  * threshold: 2x budget rate
  * WHAT IT MEANS: the availability objective consumed error budget at more than twice the sustainable rate over 30 minutes.
  * WHAT IT DOES NOT MEAN: the objective has failed; the verdict series carries that, and this is the leading indicator.
