# Runbook - Readiness flapping

Alert ids: A-10. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-10 - Readiness flapping** (severity warning, routing advisory, for 5m)
  * expression: `sum by (service) (increase(vanishgraph_readiness_state_changes_total[15m]))`
  * threshold: > 6 in 15 m
  * WHAT IT MEANS: readiness changed state more than six times in 15 minutes for one service.
  * WHAT IT DOES NOT MEAN: the dependency is unhealthy. Flapping is often the probe or the threshold, which the runbook checks first.
