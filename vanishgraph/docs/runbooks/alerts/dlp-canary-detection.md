# Runbook - Canary reached a sink

Alert ids: A-06c. Source: SPEC-007 §8 (EP-008 M6).

## What fires

* **A-06c - Canary reached a sink** (severity critical, routing page-security, for 0m)
  * expression: `sum by (sink) (increase(vanishgraph_dlp_canary_detections_total[5m]))`
  * threshold: > 0
  * WHAT IT MEANS: a canary value reached a sink: the redaction proof failed its own test.
  * WHAT IT DOES NOT MEAN: real personal data left. It means the mechanism is broken, and real data may have left the same way.
