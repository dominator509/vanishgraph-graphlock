#!/usr/bin/env sh
# Retention configuration guard (SPEC-007 §11; EP-008 M8(b)). Sentinel: `retention config: ok`
#
# WHAT IT VALIDATES, AND WHY EACH RULE EXISTS:
#
#   1. EVERY DATA CLASS §11.1 NAMES HAS AN EXPLICIT WINDOW. A missing class is a deploy-gate failure — not a warning —
#      because a class nobody configured is a class with no retention at all.
#   2. NO WINDOW IS INFINITE OR UNBOUNDED. `0`, `-1`, `infinite`, `forever`, `unlimited` and an empty string are all
#      refused: AN UNBOUNDED WINDOW IS THE ONE MISTAKE THAT SILENTLY TURNS A RETENTION POLICY INTO A PERMANENT RECORD.
#   3. THE SECURITY-RELEVANT SUBSET IS LONGER THAN THE REST and names the severity and event selectors that define it,
#      because a subset defined by nothing is a subset nobody can query.
#   4. DELETION IS BY WHOLE-PARTITION DROP, A SEALED-PARTITION REWRITE IS REFUSED, AND THE OUTCOME VOCABULARY MATCHES THE
#      METRIC CATALOGUE — an outcome the catalogue does not declare could not be recorded, so a deletion could happen
#      without a signal.
#   5. EVERY DELETION IS LOGGED AT INFO AND NEVER AT DEBUG. A retention job that runs quietly is indistinguishable from
#      one that has stopped, which is how a window becomes infinite in practice.
#
# WHAT IT DOES NOT PRETEND (§7.4-style FALLBACK, DOD-033): no partitioned store is reachable here, so the DELETION
# PROPAGATION rows — that a seeded expired record is absent afterwards, that a sealed rewrite is refused at run time —
# are recorded BLOCKED_ENVIRONMENT with the provisioning attempt, and the guard validates the DRY-RUN PLAN instead. THE
# CONFIGURATION CHECK IS NEVER RELAXED TO MAKE THE GUARD PASS: the rows it cannot execute are named, and the sentinel
# covers only what was actually checked.
set -eu
export CI=true GIT_TERMINAL_PROMPT=0 GIT_PAGER=cat PAGER=cat DEBIAN_FRONTEND=noninteractive
cd "$(dirname "$0")/.."

command -v node >/dev/null 2>&1 || { echo "retention config: FAIL - node is required but not found" >&2; exit 1; }
[ -f config/observability/retention.json ] || { echo "retention config: FAIL - config/observability/retention.json is missing" >&2; exit 1; }

EVIDENCE=.agent/evidence/EP-008/retention
mkdir -p "$EVIDENCE"

cat >"$EVIDENCE/retention-check.mjs" <<'CHECK_EOF'
...
{
  echo "retention guard - $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  echo "attempt: is a partitioned telemetry store reachable? (the expiry job drops partitions from one)"
  echo "  TRACE_BACKEND_URL=${TRACE_BACKEND_URL:-<unset>}"
  echo "  LOG_STORE_URL=${LOG_STORE_URL:-<unset>}"
  echo "  PROMETHEUS_URL=${PROMETHEUS_URL:-<unset>}"
  echo "consequence: the propagation rows (a seeded expired record absent after expiry, a sealed rewrite refused at run"
  echo "time) are BLOCKED_ENVIRONMENT per DOD-033 and are NOT counted as demonstrated; the configuration and the dry-run"
  echo "plan are validated instead, and the configuration check is never relaxed."
} >"$EVIDENCE/provisioning-attempts.txt"

if ! node "$EVIDENCE/retention-check.mjs" >"$EVIDENCE/retention-config-guard.txt" 2>&1; then
  echo "retention config: FAIL - a class, a window, the deletion rules or the access rules failed validation:" >&2
  cat "$EVIDENCE/retention-config-guard.txt" >&2
  exit 1
fi

cat "$EVIDENCE/retention-config-guard.txt"
grep -q 'verdict: OK' "$EVIDENCE/retention-config-guard.txt" || { echo "retention config: FAIL - the check printed no verdict" >&2; exit 1; }

echo "retention config: ok"
