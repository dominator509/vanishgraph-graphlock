...
const validate = (value, spec, where) => {
  ...
  // JSON SCHEMA `integer` IS A JS `number` THAT IS INTEGRAL: the first version compared the type literally and rejected
  // a verdict whose every count was an integer, which is a validator defect rather than a verdict defect.
  const matchesType = spec.type === undefined
    || spec.type === type
    || (spec.type === "integer" && type === "number" && Number.isInteger(value));
  if (!matchesType) problems.push(`${where}: expected ${spec.type} and found ${type}`);
...
ENDS_SHIP
node "$PROG" "$SCHEMA" "$GATE" "$RUN_STATE" "$REPORT" "$RESIDUAL" "$OUTDIR" "$VERIFY_OK" "$VERIFY_LAST_STAGE" "$STAGE_FAILURES" || {
  code=$?
  if [ "$code" = "3" ]; then
